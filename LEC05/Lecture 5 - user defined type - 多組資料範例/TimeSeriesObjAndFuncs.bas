...
Dim mergeDates: mergeDates = mergeTimeSeries(data2311, data2330)
Range("A1", Cells(UBound(mergeDates), "A")).Value = Application.Transpose(mergeDates)
'列印結果
'Dim printResult As Variant:
'printResult = toVariantArray(data2311)
...
Function mergeTimeSeries(seriesA As TimeSeries, _
                                                      seriesB As TimeSeries) As Variant
...
ReDim newValues(2, nDataA + nDataB) As Variant
